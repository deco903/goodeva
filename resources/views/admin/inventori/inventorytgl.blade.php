            ...
            <div class="row page-titles col-sm-12 ml-1">
                <div class="col-sm-6 p-md-0">
                    ...
                    <div class="welcome-text col-sm-3" style="margin-left:-15px;">
                       ...
                       <a href="{{route('indexgmb')}}"><button type="button" class="btn btn-light" style="margin-bottom:-10px;">GBM</button></a>
                    </div>
                </div>
                <div class="col-md-12" style="background-color: #; border: 1px solid #17202A; height: auto; margin: 10px 0px; padding: 3px; text-align: left; width: auto;">
                 ...
                 <div class="row ml-1">
                   <div class="">
                    ...
                    <div class="">
                      <form action="{{route('searchlistspn')}}" method="GET">
                        @csrf 
                    ...
                    <div>
                      <a href="{{route('indexspntgl')}}"><button class="btn btn-light mt-0" type="submit">All</button></a>
                    </div>
                 </div>            
                <div class="col-lg-12">
                        <div class="table-responsive">
                            <table class="table table-responsive-sm">
                                ...
                                <tbody >
                                @foreach($data as $value=>$keyspn)
                                  <tr align="center">
                                    <td>{{$value + $data->firstitem()}}</td>
                                    <td>{{date('d-M-y', strtotime($keyspn->created_at))}}</td>
                                    <td>{{$keyspn->nama_barang}}</td>    
                                    <td>{{$keyspn->unit}}</td>
                                    <td>{{$keyspn->stock_awal}}</td>
                                    <td>
                                        @if($keyspn->choose == '+')
                                            <p style="background-color:#77DD77;color:#FFFFFF;width:100px;"> {{$keyspn->choose}} {{$keyspn->update_stock}} </p>
                                          @elseif($keyspn->choose == ' ')
                                            <p style="color:black;width:100px;"> {{$keyspn->choose}} {{$keyspn->update_stock}} </p>
                                          @else 
                                            <p style="background-color:#FF3D33;color:#FFFFFF;width:100px;"> {{$keyspn->choose}} {{$keyspn->update_stock}} </p>
                                        @endif
                                    </td>
                                    <td> {{$keyspn->total_stock}}</td>
                                    <td>{{$keyspn->text}}</td>
                                    <td>
                                      @if($keyspn->status == 'OK')
                                         <p>{{$keyspn->status}}</p>
                                        @else
                                        <p style="background-color:#A52A2A;color:#FFFFFF;width:100px;">{{$keyspn->status}}</p>
                                      @endif
                                    </td>
                                  </tr>
                                @endforeach
